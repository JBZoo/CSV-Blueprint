...
final class ValidateCsv extends CliCommand
{
        ...
        if ($this->isCheckingSchema()) {
            ...
            foreach ($schemaFilenames as $index => $schemaFilename) {
                ...
                try {
                    ...
                    $this->out("{$prefix} Schema: {$path}");
                    $this->out("{$prefix} Exception: <yellow>{$e->getMessage()}</yellow>");
                }
            }
    ...
    private function validateCsvFiles(array $matchedFiles): array
    {
        ...
        foreach ($matchedFiles['found_pairs'] as $index => $pair) {
            ...
            $this->out("{$prefix} Schema: " . Utils::printFile($schema));
    ...
    private function printSummary(
        ...
        if (\count($matchedFiles['csv_without_schema']) > 0) {
            $this->out(
        ...
        if (\count($matchedFiles['schema_without_csv']) > 0) {
